...
description: The Configuration section allows developers to customize the Sequence Connect by passing options to the SequenceConnect wrapper.
sidebarTitle: Custom Configuration
---

Developers can customize the Sequence Connect experience by passing configuration options to the `SequenceConnect` wrapper.
...
You can customize the visual appearance of Sequence Connect by providing a custom theme object to the `defaultTheme` configuration option. Here's an example of how to implement custom styling:
...
#### `signIn.logoUrl`
<Frame>
![Enable a dark theme to Sequence Connect](/images/kit/sign-in-modal-with-logo.png)
</Frame>
...
#### `signIn.projectName`
<Frame>
![Add a project name to Sequence Connect](/images/kit/sign-in-modal-project-name.png)
</Frame>
...
Specific colors can be overwritten by passing a theme override object. The [Sequence Builder](https://sequence.build/) provides a useful playground for toying with the colors in Sequence Connect.
