...
```tsx
type UseGetTransactionHistoryReturnType = {
  data?: {
    pages: {
      transactions: Array<{
        txnHash: string
...
- `transactions`: Array of transaction objects with properties like `txnHash`, `blockNumber`, and `timestamp`
- `page`: Pagination information including current page number and whether more results exist
