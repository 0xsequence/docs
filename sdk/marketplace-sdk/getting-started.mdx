...
<Steps>
  ...
  <Step title="Set Up Providers">
    ...
    <Note>
      Note that `shadowDom` is enabled by default in the SDK configuration. This ensures that the styles are properly applied to the components when not using Tailwind CSS.
    </Note>
  ...
  <Step title="Create NFT Card Component">
    Create a new component `src/components/CollectibleCard.tsx` to display and interact with NFTs:
