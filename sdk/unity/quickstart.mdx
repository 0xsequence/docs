...
and use the following Git URL `https://github.com/0xsequence/sequence-unity.git?path=/Packages/Sequence-Unity`
  ...
  <Step title="Integrate your Login Process">
    Start by establishing a wallet session using an email OTP. Create the [Login Boilerplate](/sdk/unity/bootstrap_game#login) to send a
    one-time password to the specified email address.
