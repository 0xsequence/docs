...
You can [connect an external wallet with an embedded wallet](/sdk/wallet-linking) via a web interface.

First, you'll want to configure a server to facilitate this connection. Please [read this guide](/sdk/wallet-linking/integration) and come back once you've configured your server (you can skip the first few steps on Builder configuration and integrating embedded wallet into a web app since you've already done this on the Unity side; similarly, you can skip the sections on signing the session auth proof).
