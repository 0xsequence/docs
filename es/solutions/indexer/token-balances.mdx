---
title: Saldos de tokens
sidebarTitle: Saldos de tokens
---

## Obtenga la lista y los saldos de tokens nativos, ERC20, ERC721 y ERC1155, así como los metadatos en cualquier wallet o dirección de contrato.
...
<Note>
  **CONSEJO PRO: obtener IDs de tokens ERC721/1155**
  ...
  Notará que, por defecto, `GetTokenBalances` devolverá como máximo una instancia de token por cada contrato.
  Para obtener los saldos de tokens ERC721/1155, debe pasar el `contractAddress` al método `GetTokenBalances`.
...
## Obtenga los IDs de tokens, saldos y metadatos de colecciones ERC721 y ERC1155
_Método `GetTokenBalances` de Sequence Indexer:_
- Solicitud: POST /rpc/Indexer/GetTokenBalances
- Content-Type: application/json
- Cuerpo (en JSON):
