---
title: Inicio rápido
description: Conecta a tus usuarios con tu aplicación usando la Sequence Embedded Wallet.
...
<Steps>
  ...
  <Step title="Connect and Authenticate">
    Abre tu navegador y navega a`http://localhost:4444`. Haz clic en<b>Connect</b> para iniciar el proceso de autenticación.

    ¡Ahora puedes probar la Embedded wallet con el código base! Ahora, vamos a configurar tu propio proyecto.
  </Step>

  <Step title="Configure your Project">
    <a href="https://sequence.build/project/default/embedded-wallet?tab=config">Navega a la configuración de embedded wallet</a> para tu proyecto en Builder. Ingresa tu dirección de billetera de recuperación y establece una contraseña, luego<b>Create Configuration</b>. Ten en cuenta que esta configuración no se puede cambiar más adelante.
  </Step>

  <Step title="Update Environment Variables">
    Navega al directorio clonado y copia y pega esta configuración en el archivo`.env` reemplazando la configuración existente.

    Asegúrate de actualizar el`PROJECT_ACCESS_KEY` y`WAAS_CONFIG_KEY` con las nuevas claves ubicadas en la<a href="https://sequence.build/project/default/embedded-wallet?tab=integration">página de integración</a>.
  ...
  <Step title="Test your new configuration">
    Guarda tu`.env` archivo, ejecuta`pnpm dev` desde el directorio raíz, y navega de nuevo a`http://localhost:4444`. Inicia sesión en tu billetera usando el correo electrónico para probar las acciones de la billetera nuevamente.

    ¡Has terminado! Ahora puedes ver tu conexión de billetera en la página de Resumen y las claves están limitadas a tu proyecto.
  </Step>
</Steps>

Como siguientes pasos:

* Ahora puedes configurar varios[proveedores de autenticación social](/solutions/builder/embedded-wallet#login-providers).
* [Usar más características de nuestro web-sdk](/sdk/web/guides/send-sponsored-tx).
* [Solicitar un token de ID de Sequence para validar usuarios en tu backend](/sdk/headless-wallet/verification).
