...
También deberá configurar variables específicas de Sequence a través del Web-SDK, sobre lo cual puede obtener más información [aquí](/sdk/web/overview).

Luego, solicite el idToken de Sequence una vez que el usuario esté autenticado y páselo a su backend siguiendo esta [guía](/sdk/web/guides/verification).

A partir de aquí, continúe con el [Paso 2](/solutions/wallets/developers/embedded-wallet/migration#recommended-migration-process) según su backend y base de datos de usuarios para hacer seguimiento al proceso de migración de sus usuarios.
