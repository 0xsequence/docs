...
## 1. Minteo
El primer paso es crear un coleccionable desde Sequence Builder y mintear algunos tokens, lo cual puede lograrse con esta [guía](/solutions/builder/contracts/deploy-an-item-collection). Luego, utilice el `tokenId` que minteó en los siguientes pasos para consultar y completar órdenes.
...
Marketplace SDK es un conjunto de herramientas completo que integra de manera sencilla nuestros Marketplaces en sus aplicaciones. Más información sobre [Marketplace SDK](/sdk/web/marketplace-sdk/overview)
