...
Primero, siga [esta guía](/solutions/builder/project-management) para aprender cómo registrarse en el [Sequence Builder Console](https://sequence.build/) y crear un proyecto.

Para usar ciertas funciones, como `Gas Sponsoring` y el uso de la `Transactions API`, deberá actualizar su plan de proyecto a `Developer` [siguiendo esta guía](/solutions/builder/project-settings#5-billing-settings)
...
Para desplegar un contrato, puede seguir [esta guía](/solutions/builder/contracts/deploy-an-item-collection) para desplegar su ERC1155 y actualizar su `wrangler.toml` con el `CONTRACT_ADDRESS`.
...
Para permitir que la Transactions API retransmita transacciones sin comisión, patrocine el gas siguiendo [esta guía](/solutions/builder/gas-sponsorship#watch-gas-sponsorship-in-action-in-builder) para el contrato desplegado.
