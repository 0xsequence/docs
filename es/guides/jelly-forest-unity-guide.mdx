...
Descubra [por qué usar smart contract wallets aquí](/solutions/technical-references/smart-contract-wallets)

Conozca [qué es un embedded wallet aquí](/solutions/wallets/developers/embedded-wallet/overview)
...
3. [Cree un proyecto para su juego en el Builder Console](/solutions/builder/project-management)
...
El LoginPanel se encargará de toda la lógica de inicio de sesión con redes sociales por usted. Si le interesa saber cómo está implementado, puede revisar las implementaciones de `LoginPage` y `OpenIdAuthenticator`.
...
Para más información sobre cómo funciona la autenticación en la solución Embedded Wallet de Sequence, consulte nuestra [documentación](/solutions/wallets/developers/embedded-wallet/overview) y [entrada de blog](https://sequence.xyz/blog/sequence-embedded-wallets).
...
Leer los tokens en la wallet de un usuario es un proceso complicado que se vuelve mucho más sencillo usando el [Indexer de Sequence](/api-references/indexer/overview), que está implementado en el [Unity SDK](/sdk/unity/indexer/read-from-blockchain).
...
:::danger
Advertencia: el smart contract `BurnToMint` compartido arriba no ha sido auditado por un tercero. ¡Reutilícelo con precaución!
:::
