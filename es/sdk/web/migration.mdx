...
description: Web SDK (v4) es ahora Web SDK (v5), aquí están los cambios más importantes.
...
Todas nuestras bibliotecas de hooks han sido renombradas, aquí están las nuevas dependencias que necesitas para comenzar con Web SDK:
...
* `@0xsequence/kit` -> `@0xsequence/wallet-widget` para interfaz de billetera
* `@0xsequence/kit-checkout` -> `@0xsequence/checkout`
* no es necesario importar manualmente el`@0xsequence/design-system/styles.css` cuando se usa`@0xsequence/connect`
* `@0xsequence/connect` es compatible con`@0xsequence/design-system` v2
* `KitProvider` es ahora`SequenceConnectProvider`
* `KitCheckoutProvider` es ahora`SequenceCheckoutProvider`
* `KitWalletProvider` es ahora`SequenceWalletProvider`
