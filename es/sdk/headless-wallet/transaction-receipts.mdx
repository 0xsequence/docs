...
description: El contenido explica la generación de recibos de transacción para
  transacciones exitosas y fallidas.
sidebarTitle: Recibos de transacción
---

**Recibos de Transacción:** Las transacciones devuelven recibos indicando éxito o fallo. Los recibos fallidos se generan sin ejecución de la transacción.

**Respuesta de Transacción Exitosa:** Una transacción exitosa proporciona una SentTransactionResponse. Siempre verifique esta respuesta con el **`isSentTransactionResponse`** guardia de tipo antes de acceder al hash de la transacción.
...
* **receipt.logs**: Registros emitidos durante la transacción.
* **receipt.status**: Indica si la transacción SUCCEEDED o FAILED.
* **receipt.revertReason**: Explica por qué una transacción fue revertida, si aplica.

Para ver el objeto de respuesta completo, consulte la sección [Cargas de respuesta](/solutions/wallets/embedded-wallet/architecture/response-payloads).
...
Una billetera In-Game anticipa las transacciones fallidas simulándolas en la red antes del envío real. Si la simulación falla, la transacción no se envía y se devuelve una FailedTransactionResponse.

Para ver el objeto de respuesta completo, consulte la sección de [Cargas de respuesta](/solutions/wallets/embedded-wallet/architecture/response-payloads) de la billetera.
