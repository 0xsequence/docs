...
- [Autenticación con X (Twitter)](/sdk/headless-wallet/authentication#authenticating-with-x-twitter)
...
<Note>
  Asegúrese de [completar los detalles de configuración](/solutions/builder/embedded-wallet/x-configuration) para X (Twitter) previamente.
</Note>

Para autenticar a un usuario con X (antes Twitter), primero debe obtener un access token a través del flujo OAuth 2.0 de X. Debido a ciertas complejidades con la API de X, recomendamos utilizar el Sequence X Auth Proxy como se detalla en la guía de configuración.

Una vez que tenga el access token del usuario, puede pasarlo al método `signIn` usando la propiedad `xAccessToken`:

```typescript
const response = await sequence.signIn({
  xAccessToken: 'THE_USER_X_ACCESS_TOKEN'
}, 'X session');
```

A diferencia de otros proveedores que utilizan un `idToken` (OIDC), para la autenticación con X deberá proporcionar un `xAccessToken`. Consulte la [guía de configuración de X (Twitter)](/solutions/builder/embedded-wallet/x-configuration) para más detalles.
