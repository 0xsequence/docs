...
3.  **Acceso desde teléfono móvil** mediante soporte de [WalletConnect](https://walletconnect.com/) dentro de Sequence Wallet: los usuarios pueden comunicarse con sus wallets de forma remota usando el excelente protocolo Wallet Connect. Esta es una opción ideal si usa Sequence Wallet desde su teléfono móvil y quiere conectar su wallet a una aplicación de escritorio.
...
Esto significa que, si ha desarrollado una aplicación para funcionar con MetaMask, Sequence también funcionará sin cambios. Si está usando web3.js o ethers.js, Sequence también funcionará. Así es la interoperabilidad en web3 :)

## Conectando su aplicación con `0xsequence`

<Note>
  \[Un wallet increíble sin que sus usuarios tengan que instalar nada :D]
  Al integrar `0xsequence`, los usuarios pueden acceder a su aplicación sin instalar extensiones especiales, ¡y el wallet también funciona en navegadores móviles!
</Note>

Su aplicación puede conectarse al wallet del usuario instanciando primero el proveedor de Wallet:
...
Las aplicaciones con integración directa de Sequence pueden especificar un objeto `ConnectOptions` al ejecutar `wallet.connect()`.
...
#### **app**
Nombre de la aplicación que se mostrará al usuario en la pantalla de conexión.
...
#### **authorize**
`authorize` realizará una firma ETHAuth eip712 y devolverá la prueba a la aplicación.
...
#### **settings.bannerUrl**
URL de una imagen de banner que los usuarios verán al conectar o iniciar sesión en su aplicación. La imagen debe tener una relación de aspecto 3:1, donde 1200x400 funciona mejor.
...
## Conectando su aplicación con `web3.js` o `ethers.js`
Para ver un ejemplo completo de una aplicación que admite Sequence (on-demand + extensión de Chrome), Metamask y WalletConnect, consulte el [repositorio Demo-Dapp-Web3Modal](https://github.com/0xsequence/demo-dapp-web3modal).

<Frame caption="Sequence Web3Modal Integration">
  ![Integración de Sequence con Web3Modal](/images/web3modal.png)
</Frame>

## Conectando a cualquier aplicación de Ethereum con la extensión de Chrome de Sequence Wallet
...
Sequence ya permite conectarse a aplicaciones mediante [WalletConnect](https://walletconnect.com).
Si su aplicación ya soporta WalletConnect y no necesita funciones específicas de Sequence, no es necesario hacer nada más.
...
Se escanea el código QR de la aplicación.
Como alternativa, el código se puede pegar desde el portapapeles del sistema operativo si el usuario lo eligió previamente.
...
La conexión fue exitosa y la aplicación se actualiza para reflejarlo.

<Frame caption="A success message for Wallet Connect">
  <img src="/images/walletconnect/success.png" />
</Frame>

Mientras está conectado, la aplicación puede hacer solicitudes de firma a Sequence.
Sequence siempre solicitará confirmación al usuario para cualquier actividad iniciada por la aplicación.

<Frame caption="Confirmation by user activity to initiate a app transaction">
  <img src="/images/walletconnect/activity.png" />
</Frame>

Una vez que el usuario termina de usar la aplicación, puede desconectar el wallet desde el menú de sesión.
