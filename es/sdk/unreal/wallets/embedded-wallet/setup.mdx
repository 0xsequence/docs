---
title: Configuración de Embedded Wallet en Unreal
description: Documentación de inicio rápido para el SDK de Unreal de Sequence.
sidebarTitle: Configuración
---

<Steps>
  <Step title="Configure su proyecto de Unreal">
    Cree un proyecto en [Sequence Builder](https://sequence.build) y descargue su archivo de configuración como se muestra a continuación.

    <Frame>
      <img src="/images/unreal/builder_config_file.png" />
    </Frame>

    Asegúrese de completar los campos requeridos:

    - **ProjectAccessKey:** Su Project Access Key de su [proyecto en Builder.](https://sequence.build/)
  </Step>

  <Step title="Integrar inicio de sesión">
    Comience creando el widget [Sequence Login Window](/sdk/unreal/bootstrap#login). Esto le da acceso rápido a nuestros métodos de inicio de sesión. Consulte la sección de [autenticación](/sdk/unreal/wallets/embedded-wallet/onboarding/authentication#requirements) para saber cómo configurar cada método de inicio de sesión.

    Aprenda cómo [integrar manualmente métodos de inicio de sesión](/sdk/unreal/wallets/embedded-wallet/onboarding/authentication) como Google, Apple o PlayFab.

    <Frame>
      <img src="/images/unreal/boilerplate_login.png" />
    </Frame>
  </Step>

  <Step title="Inicie su juego">
    El SDK de Unreal de Sequence incluye varios Boilerplates para ayudarle a iniciar su juego rápidamente. Una vez que todo esté configurado, puede crear widgets para mostrar un Inventario, una vista de wallet o una tienda dentro del juego. Vea [cómo integrar un perfil de jugador](/sdk/unreal/bootstrap#player-profile) usando solo tres nodos de Blueprint para ver y enviar fondos.

    <Frame>
      <img src="/images/unreal/boilerplate_profile.png" />
    </Frame>
  </Step>
</Steps>