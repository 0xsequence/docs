...
<CardGroup>
  <Card title="ThirdWebからSequenceへの移行" icon="book" href="/solutions/wallets/embedded-wallet/migration#how-to-migrate-from-thirdweb-to-sequence">
    このガイドに従って、ThirdWeb In-App WalletsからSequenceへの移行を行ってください。
  </Card>
</CardGroup>

## 推奨される移行手順
この移行プロセスは、ユーザーの体験にほとんど影響を与えずに、既存のウォレットプロバイダーからSequence Embedded Walletsへスムーズに移行できるよう設計されています。このプロセスは、移行が実施された後、ユーザーが初めてログインした際に自動的に行われ、一般的には以下のような流れになります。
