...
移行体験に特化した内容をお探しの場合は、ユーザーがNFTを閲覧したり、チェーンを切り替えたり、Thirdwebアカウントから別のウォレットへ資産を送金できる[専用のサンプルリポジトリ](https://github.com/0xsequence-demos/embedded-wallet-migration)もご用意しています。
