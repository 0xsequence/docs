---
title: クイックスタート
description: Sequence 組み込みウォレットでユーザーをアプリに接続します。
...
<Steps>
  <Step title="Clone Starter Repo">
    ターミナルから以下のコマンドを実行してスターターリポジトリをクローンします。
  ...
  <Step title="Connect and Authenticate">
    ブラウザを開いて次のURLに移動します`http://localhost:4444`。<b>接続</b>をクリックして認証プロセスを開始します。

    これでボイラープレートで組み込みウォレットをテストできます！次に、あなた自身のプロジェクトを設定しましょう。
  </Step>

  <Step title="Configure your Project">
    <a href="https://sequence.build/project/default/embedded-wallet?tab=config">組み込みウォレットの設定に移動</a>してBuilderでプロジェクトを設定します。リカバリーウォレットアドレスを入力してパスワードを設定し、<b>設定を作成</b>してください。この設定は後から変更することはできませんのでご注意ください。
  </Step>

  <Step title="Update Environment Variables">
    クローンしたディレクトリに移動し、この設定をファイルにコピー＆ペーストします`.env`既存の設定を置き換えてください。

    必ず`PROJECT_ACCESS_KEY`と`WAAS_CONFIG_KEY`を<a href="https://sequence.build/project/default/embedded-wallet?tab=integration">統合ページ</a>にある新しいキーで更新してください。
  ...
  <Step title="Test your new configuration">
    ファイル`.env`を保存し、`pnpm dev`をルートディレクトリから実行し、`http://localhost:4444`に戻ります。メールを使用してウォレットにサインインし、ウォレットのアクションを再度テストしてください。

    これで完了です！概要ページでウォレット接続を確認でき、キーはプロジェクトにスコープされています。
  </Step>
</Steps>

次のステップとして：

* 様々な[ソーシャル認証プロバイダー](/solutions/builder/embedded-wallet#login-providers)を設定できます。
* [web-sdkのより多くの機能を使用](/sdk/web/guides/send-sponsored-tx)できます。
* [バックエンドでユーザーを検証するためにSequenceからIDトークンをリクエスト](/sdk/headless-wallet/verification)できます。
