...
このガイドでは、メールやソーシャルログインなど従来の認証を使って[埋め込みウォレット](/solutions/wallets/developers/embedded-wallet/overview)にサインインする方法を紹介します。その後、Web SDK を利用して外部ウォレットアドレスを埋め込みウォレットにリンクするため、署名を使ってオフチェーンでプライベートに管理される単一ユーザープロファイルのアドレスセットを作成します。さらに、API の他の機能（リンク済みウォレットの一覧表示やリンク状態の削除など）も、プライバシー保護のため署名を利用して解説します。
...
## 4. 連携済みウォレットの解除
最後に、ウォレットを親ウォレットからオフチェーンで連携解除したい場合、両方のウォレット（Embedded WalletとWeb SDK連携ウォレット）で署名を行い、連携ウォレットアドレスを[ステップ6と同様に](/solutions/wallets/developers/embedded-wallet/wallet-linking#2-link-an-external-wallet-to-an-embedded-wallet)渡すことで、連携状態を解除できます：
