---
title: セッション権限
description: アプリで承認や送金の権限をリクエストするよう設定し、ユーザーへのウォレットポップアップの繰り返し表示をなくしましょう。
sidebarTitle: セッション権限
---

Smart Sessionsは、必要となる前にユーザーのウォレットから特定の権限をリクエストできる強力な機能です。権限リクエストは接続リクエストに組み込まれているため、認証と権限リクエストを一度に行えます。その後、発行されたセッションキーを使って、定義したルールのもとで追加のポップアップなしにトランザクションを実行できます。

## 🔧 完全な設定例
Web SDKを使って、アプリで承認や送金の権限をリクエストする設定方法の完全な例を紹介します。
