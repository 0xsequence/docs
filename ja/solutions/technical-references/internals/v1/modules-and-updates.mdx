...
`MainModule`はすべてのSequenceウォレットの初期モジュールです。他のモジュールと異なり、署名者のセットをコントラクトストレージに保存せず、[salt](/solutions_old/technical-references/internals/v1/wallet-factory)をFactoryコントラクトに渡して利用します。
...
[ウォレットアドレスの計算方法](/solutions_old/technical-references/internals/v1/wallet-configuration) をご覧ください。