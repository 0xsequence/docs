...
`MainModule`はすべてのSequenceウォレットの初期モジュールです。他のモジュールと異なり、署名者のセットをコントラクトストレージに保存せず、[salt](/solutions/technical-references/architecture/v1/wallet-factory)をFactoryコントラクトに渡して利用します。
...
[ウォレットアドレスの計算方法](/solutions/technical-references/architecture/v1/wallet-configuration) をご覧ください。