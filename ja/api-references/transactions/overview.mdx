...
最大の特長は、Sequence Transactions APIを使ったトランザクションは、既存のEthereumコントラクトと互換性があり、Sequence Relayerを統合する際にコントラクトやアプリを変更する必要がないことです。

Sequence Transactions APIはフロントエンドアプリだけでなく、バックエンドでも利用できます。ご希望の言語に合わせて、[Typescript](/sdk/typescript/overview)や[Go](/sdk/go/overview)など、対応するSDKをインストールしてください。
