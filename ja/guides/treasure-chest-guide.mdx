...
コントラクトのデプロイ方法は[こちらのガイド](/solutions/builder/contracts/deploy-an-item-collection)を参考にERC1155をデプロイし、`wrangler.toml` に `CONTRACT_ADDRESS` を設定してください。
...
Transactions API で手数料なしでトランザクションをリレーできるようにするには、[こちらのガイド](/solutions/builder/gas-sponsorship#watch-gas-sponsorship-in-action-in-builder)に従い、デプロイ済みコントラクトのガススポンサー設定を行ってください。
