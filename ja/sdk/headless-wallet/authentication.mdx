...
- [X（Twitter）での認証](/sdk/headless-wallet/authentication#authenticating-with-x-twitter)
...
<Note>
  事前に[X（Twitter）の設定詳細](/solutions/builder/embedded-wallet/x-configuration)を完了してください。
</Note>

X（旧Twitter）でユーザーを認証するには、まずOAuth 2.0フローを通じてアクセストークンを取得する必要があります。XのAPIにはいくつか複雑な点があるため、設定ガイドで説明しているSequence X Auth Proxyの利用を推奨します。

ユーザーのアクセストークンを取得したら、`xAccessToken`プロパティを使って`signIn`メソッドに渡します。
...
他のプロバイダーが`idToken`（OIDC）を使用するのとは異なり、X認証では`xAccessToken`を指定する必要があります。詳細は[X（Twitter）設定ガイド](/solutions/builder/embedded-wallet/x-configuration)をご参照ください。
