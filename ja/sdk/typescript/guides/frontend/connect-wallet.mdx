...
3.  **モバイル端末からのアクセス**：[WalletConnect](https://walletconnect.com/)対応により、Sequence Wallet内でユーザーはWallet Connectプロトコルを使ってリモートでウォレットと通信できます。モバイル端末でSequence Walletを利用し、デスクトップアプリと接続したい場合に最適な方法です。
...
つまり、MetaMask対応のアプリを開発していれば、Sequenceでもそのまま動作します。web3.jsやethers.jsを使っている場合も、Sequenceはそのまま利用できます。これがweb3における相互運用性の魅力です。

## アプリを`0xsequence`と接続する

<Note>
  \[ユーザーが何もインストールせずに使える快適なウォレット :D]
  `0xsequence`を統合することで、ユーザーは特別な拡張機能をインストールせずにアプリへアクセスでき、ウォレットはモバイルブラウザにも対応しています！
</Note>

まず、ウォレットプロバイダーをインスタンス化して、アプリをユーザーのウォレットと接続できます。
...
## ウォレットのログインと接続オプション
Sequenceを直接統合したアプリでは、`wallet.connect()`実行時に`ConnectOptions`オブジェクトを指定できます。
...
#### **authorize**
`authorize`はETHAuth eip712署名を実行し、証明をアプリに返します。
...
#### **settings.bannerUrl**
アプリ接続やログイン時にユーザーが目にするバナー画像のURLです。バナー画像は3:1のアスペクト比（1200x400が最適）を推奨します。
...
## アプリを`web3.js`または`ethers.js`と接続する
Sequence（オンデマンド＋Chrome拡張）、Metamask、WalletConnectに対応したアプリの完全な例については、[Demo-Dapp-Web3Modalリポジトリ](https://github.com/0xsequence/demo-dapp-web3modal)をご覧ください。

<Frame caption="Sequence Web3Modal Integration">
  ![Sequence Web3Modal連携](/images/web3modal.png)
</Frame>

## Sequence Wallet Chrome拡張機能を使って、あらゆるEthereumアプリと接続する
...
Sequenceはすでに[WalletConnect](https://walletconnect.com)経由でのアプリ接続に対応しています。
アプリがWalletConnectに対応していて、Sequence固有の機能が不要な場合は、追加の対応は不要です。
ユーザーから見たWalletConnectの流れは以下の通りです。
...
アプリのQRコードをスキャンします。
または、以前に選択した場合はOSのクリップボードからコードを貼り付けます。
