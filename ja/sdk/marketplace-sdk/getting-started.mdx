...
description: Marketplace SDK をアプリケーションに統合する方法について、インストール・設定・主要なマーケットプレイス機能（オファー作成、リスティング作成、取引処理など）の実装まで、包括的に解説します。
sidebarTitle: はじめに
---
...
<Steps>
  ...
  <Step title="プロバイダーのセットアップ">
    ...
    <Note>
      SDK 設定の `shadowDom: true` オプションは、Tailwind CSS を使用しない場合に必要です。これにより、スタイルがコンポーネントに正しく適用されます。
    </Note>
  </Step>
  ...
  <Step title="NFT カードコンポーネントの作成">
    NFT を表示・操作するための新しいコンポーネント `src/components/NFTCard.tsx` を作成します。
