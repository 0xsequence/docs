...
ユーザーがマーケットプレイスに出品できるようにするには、まず [ブロックチェーンからデータを取得する](/sdk/unity/indexer/read-from-blockchain) ことで、ユーザーのトークン残高を確認します。ユーザーのトークン残高を取得したら、その情報を UI 上でユーザーに表示するのが一般的です。
