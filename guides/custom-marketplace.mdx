...
The first step is to create a collectible from the Sequence Builder and mint a few tokens, which can be accomplished with this [guide](/solutions/builder/contracts/deploy-an-item-collection) and to use the `tokenId` you minted in the following steps to query and fulfill orders.
...
Either you can start from a great foundation with our [Marketplace Boilerplate](/solutions/payments/marketplace/custom), which already includes all of this integrated with a great UI, or we will walk you through how to use React from scratch here.
...
Marketplace SDK is a comprehensive toolkit that seamlessly integrates our Marketplaces into applications. More about [Marketplace SDK](/sdk/web/marketplace-sdk/overview)
