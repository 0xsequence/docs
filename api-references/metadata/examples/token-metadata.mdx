...
* [Learn how to query token metadata directly for any contract](/api/metadata/examples/token-metadata#fetch-token-metadata-for-any-erc721-or-erc1155-contract)
* [Learn about token metadata standards and formats for ERC721 and ERC1155 tokens](/api/metadata/examples/token-metadata#token-metadata-standards)
...
1. Check current token metadata using [token metadata fetch](/api/metadata/examples/token-metadata#fetch-token-metadata-for-any-erc721-or-erc1155-contract)
2. Modify token metadata content either onchain or via token baseURI 
3. Call refresh token ID endpoints using [refresh metadata](/api/metadata/examples/token-metadata#refreshing-token-metadata)
4. Check token metadata changes using [token metadata fetch](/api/metadata/examples/token-metadata#fetch-token-metadata-for-any-erc721-or-erc1155-contract)
...
On Sequence Metadata, we support both structures for token contracts which use either format for their [properties/attributes](/api/metadata/examples/token-metadata#attributes-vs-properties).
...
by OpenSea and described in more detail in our [Contract metadata](/api/metadata/examples/contract-metadata) section.
