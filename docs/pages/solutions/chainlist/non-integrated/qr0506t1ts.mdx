--- 
title: QR0506T1TS - Avalanche Blockchain Network
description: Explore QR0506T1TS, a blockchain network with chain ID 934567. Learn about its native currency, QR0506T1TS Token, and how to interact with the network.
---

# QR0506T1TS

QR0506T1TS is a blockchain network with chain ID 934567.

## Network Details

- **Chain ID**: 934567
- **Chain Name**: Avalanche
- **Short Name**: QR0506T1TS
- **Network ID**: 934567
- **Currency**: 
  - **Name**: QR0506T1TS Token
  - **Symbol**: VPE
  - **Decimals**: 18

## RPC URLs

QR0506T1TS can be accessed through the following RPC endpoints:

- https://subnets.avax.network/qr0506t1ts/testnet/rpc

## QR0506T1TS Block Explorers



## Additional Information

- **Official Website**: Not provided

## Sequence Integration Status

This network does not yet have Sequence support but is available for deployment.

## QR0506T1TS Faucet

When building with Sequence, faucets aren't necessary due to our smart account infrastructure. This allows for a smoother onboarding process for your users and a better developer experience.
