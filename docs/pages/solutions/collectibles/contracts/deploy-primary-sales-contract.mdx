...
Navigate to the previously deployed Collectibles contract, select the Write Contract tab, and call the `grantRole` function to grant the Sale contract the `MINTER_ROLE`.
...
Once all that information is filled out, go ahead and click Write and submit the transaction to mint the token!
::::

What's next? You can now utilize the sale contracts publicly on your website for an NFT drop or use these contracts for your in-game store to sell game items to your players. We also recommend taking a look at our boilerplates that leverage your sale contracts for various use cases such as Fiat payments or ability to pay in any currency for your sale.
