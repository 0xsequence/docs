...
Copy the Sales contract address and navigate to the previously deployed Collectibles contract, select Settings > Permissions, then click `Add Collaborators`. Paste in the address of the Sales contract and update the Permission to `Minter`, then select `Add Collaborator` to sign a transaction to update the Access Control.
