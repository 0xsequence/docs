...
Next, a configuration variable for Sequence Kit will need to be created as either a `waas` (meaning an [Embedded Wallet](http://localhost:5173/solutions/wallets/embedded-wallet/overview)) or `universal` (meaning a [Universal Wallet](http://localhost:5173/solutions/wallets/embedded-wallet/overview)) wallet type.

For `waas`, first obtain a [WaaS Config Key from the Sequence Builder](/solutions/builder/embedded-wallet/) and setup other [Login Provider configuration](/solutions/builder/embedded-wallet/#login-providers). For both wallet type options, [obtain and use a project access key](/solutions/builder/getting-started).

```typescript [config.ts]
import { KitConfig, createConfig } from '@0xsequence/kit'

export const config: KitConfig = createConfig('waas' /*or, 'universal'*/, {
...
In order to customize further, [you can view additional configuration parameters.](https://github.com/0xsequence/kit?tab=readme-ov-file#setting-up-the-library)
...
To add the optional Inventory feature, add the `KitWalletProvider` below the `KitProvider` wrapper.
