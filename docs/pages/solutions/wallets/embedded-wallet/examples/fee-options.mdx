---
title: Title: Implementing Fee Options for Gas Fees with Sequence Embedded Wallet
...
- **Sponsored Contract or Wallet**: Having the smart contract or wallet sponsored, not requiring a fee option inputted into the SDK call. You can learn about sponsoring a contract or wallet [here](/solutions/builder/gas-tank).
- **Fee Options**: Using a Sequence API to get a custom fee quote and fee options object, pass both into the waas sdk call. [See below](/solutions/wallets/embedded-wallet/examples/fee-options#implementing-fee-options).
...
You can either transfer tokens to your wallet, or, use the [on-ramp feature](/solutions/wallets/sequence-kit/on-ramp) using the Sequence Kit solution.

And ensure that the following packages are installed:

```shell
pnpm install @0xsequence/waas ethers@5.7.2
```
