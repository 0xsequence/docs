---
title: Authentication with Sequence Embedded Wallet
...
<Callout type='info'>
View the [complete configuration details and screenshots page](/solutions/builder/embedded-wallet/stytch-configuration) for more information.
<Callout>

Get started with our starter boilerplate using the [Sequence CLI](https://github.com/0xsequence/sequence-cli/) and follow the prompts with either your own Stytch configuration keys [setup in their dashboard](https://stytch.com/dashboard/start-now) or default configuration keys and tokens for testing:

```shell
npx sequence-cli boilerplates stytch-embedded-wallet-react-boilerplate
```

### Custom Implementation

Or, use the typical approach to `JWT` authentication with an inputted `JWT` `idToken` into the Embedded Wallet SDK.

In order to retrieve the `JWT` for the signed in session for web using `@stytch/react`, use the following package `browser-cookies` to retrieve the `JWT` from the cookies after the callback for the redirect URI has been completed:

And implement like such:
