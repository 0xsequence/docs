...
- [Authentication with Social Provider](/solutions/wallets/embedded-wallet/examples/use-wallets#authenticated-embedded-wallet)
- [Authentication via Email](/solutions/wallets/embedded-wallet/examples/use-wallets#guest-embedded-wallet)
- [Authentication as a Guest](/solutions/wallets/embedded-wallet/examples/use-wallets#guest-embedded-wallet)
...
### Account Federation
If a user tries to authenticate with the same credentials across different providers, for example the same address for email and Google, you will receive a notification that the account already exists. In this case, we recommend linking the user's accounts through [Account Federation](/solutions/wallets/embedded-wallet/examples/account-federation#embedded-wallet-account-federation) which ensures a single address across multiple providers.
...
#### Authenticating with Google Login
For example, with Google Login and leveraging the `@react-oauth/google` package:
