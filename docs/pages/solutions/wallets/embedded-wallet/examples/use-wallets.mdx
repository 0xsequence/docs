...
- [**Initialize SDK & Perform Authentication**](/solutions/wallets/embedded-wallet/examples/use-wallets#initialize-sdk--perform-authentication): Ensure the SDK is initialized and the user is authenticated via one of our offered social options to begin using WaaS functions.
...
## Initialize SDK & Perform Authentication

Before you can use the Sequence WaaS SDK, you need to acquire the following configuration keys from the Sequence Builder:
...
Then, you'll need to input the [JWT](https://jwt.io/introduction) `idToken` from the social auth method, usually looking in the form of `eyJh...`:
