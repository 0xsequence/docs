...
The Embedded Wallet can be initialized using a Social Authentication `OAuth` token (or [PlayFab `ticket`](/solutions/builder/embedded-wallet/playfab-configuration)) for web2 authentication and web3 features, or, as a `Guest Wallet` for ephemeral wallet functions, still with being web3 on-chain.
