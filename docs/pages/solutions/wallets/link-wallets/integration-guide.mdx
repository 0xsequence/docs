...
description: This guide demonstrates how to integrate a parent Embedded Wallet with web2 authentication methods for wallet linking purposes. It covers signing into a social login provider and linking using signatures from an external wallet signed in via Sequence Kit stored privately and securely.
---
...
3. [Integrate Sequence Kit with an Embedded Wallet](/solutions/wallets/link-wallets/integration-guide#3-integrate-sequence-kit-with-an-embedded-wallet)
4. [Sign In with Sequence WaaS NPM Package](/solutions/wallets/link-wallets/integration-guide#4-sequence-waas-npm-package)
...
```shell
npx sequence-cli boilerplates create-embedded-wallet-linking-starter
```
...
## 3. Integrate Sequence Kit with an Embedded Wallet

Quickstart yourself with an Sequence Kit React based application using the Sequence CLI, using the following command in your terminal:

```shell
npx sequence-cli boilerplates create-embedded-wallet-react-starter
```

This command inputted in your terminal will present options to input the `Waas Config Key` and `Project Access Key` you received in [step 2](/solutions/wallets/link-wallets/integration-guide#2-access-key-management), after, you'll need to set up your `Client IDs` (if using social) for the Login Providers.

<Callout type='warning'>
You will need to comment out the WaaS Connector that corresponds to the parent wallet you're using for the Embedded Wallet sign in type in the next step in `App.tsx` to not create a wallet linking loop.
</Callout>

## 4. Sequence Waas NPM Package

Next, integrate the `@0xsequence/waas` package into your application using the [following guide](/solutions/wallets/embedded-wallet/examples/use-wallets#get-wallet-address) and use a corresponding React package to sign a user in to obtain a `JWT Id Token`, and name the variable `sequenceWaas` used in the following steps. 

Here is an example project for [Google & Apple](https://github.com/0xsequence-demos/template-embedded-wallet-web2-auth) and [Email that uses a One-Time Password code](https://github.com/0xsequence/email-embedded-wallet-react-boilerplate).
...
In order to link a Sequence Kit signed in wallet (e.g. EOA, Sequence Universal Wallet, etc.) to an Embedded Wallet, begin with the following API function `linkWallet`, the typed arguments and return types (negating the optional values) assuming you already have a Seqeunce kit enabled application:
