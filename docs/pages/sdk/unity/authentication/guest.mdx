...
Reminder: since guest login users have no persisting credentials, when the session ends, the user will be unable to access their embedded wallet again. Before ending the session, be sure to prompt the user to [federate their account](/sdk/unity/federated-accounts) and associate a login method + credentials with it for subsequent sessions!
