...
description: ja-translated Documentation for Unreal SDK API for writing to the blockchain with the Sequence infrastructure stack for web3 gaming.
---
...
## MOCKDATA + ja-translated Sign Message
ja-translated Sign a given message as a hex string. For example, you can use this signature to validate content on your backend.
...
## MOCKDATA + ja-translated Send Native Tokens
ja-translated Send native token balances to other users, such as sending ETH on the Ethereum Mainnet.
...
MOCKDATA + ja-translated Send your custom ERC20 tokens from Builder to users, or transfer existing tokens like USDC or WETH.
...
## ja-translated Send NFTs (ERC721 Tokens)
MOCKDATA + ja-translated Enable your users to send NFTs to others.
...
## ja-translated Send Collectibles (ERC1155 Tokens)
MOCKDATA + ja-translated Enable your users to send Collectibles to others.
...
## ja-translated Send Raw Transactions
MOCKDATA + ja-translated _Note: if you want call contracts with the Raw type you'll want include the header #include "ABI/ABI.h" in order to use the ABI to encode the data for a contract call._
...
## ja-translated Send Delayed Encode Transactions
MOCKDATA + ja-translated When working with Blueprints, it is easier to call a contract via server-side encoding using a Delayed Encode transaction.
...
## ja-translated Send Transactions with Fee Options
MOCKDATA + ja-translated To send transactions with fee options, first retrieve the available fee options using the `GetFeeOptions` method.
