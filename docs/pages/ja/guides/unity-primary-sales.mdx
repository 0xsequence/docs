---
title: ja-translated How to do Primary Sales for On-Chain Items in Unity
description: ja-translated This guide covers the creation of a Primary Sale with Sequence's Unity SDK.
---

# ja-translated How to do Primary Sales for On-Chain Items in Unity
MOCKDATA + ja-translated Accelerate your game growth by selling items directly to your players. In this guide, we will go over the steps how
...
- MOCKDATA + ja-translated [Primary Sales Contract](/solutions/collectibles/contracts/deploy-primary-sales-contract/): How to set up and deploy contracts for launching a primary sale — suitable for an Web Shop, NFT Drop, and more.
- MOCKDATA + ja-translated [Sequence Indexer](solutions/builder/indexer): Leveraging the Sequence Indexer to query NFT metadata and user's wallet assets.

## ja-translated 1. Deploy your own Token- and Sale Contract in Sequence Builder
MOCKDATA + ja-translated We first need a Primary Sales Contract along with an ERC1155 contract that will contain our game items we want to sell.
To do that, please follow the [guide](/solutions/collectibles/contracts/deploy-primary-sales-contract) here.

## ja-translated 2. Use the Primary Sales Demo inside Sequence's Unity SDK
MOCKDATA + ja-translated To get started, import [Sequence's Unity SDK](https://github.com/0xsequence/sequence-unity/releases) into your project.
...
## ja-translated 3. Set your Sale Configurations in your Project
MOCKDATA + ja-translated To configure your sale, use the `PrimarySalePage` object to input your sale information.
In the Configuration section, you’ll find the following options:
- ja-translated Chain: Select the chain where your contract is deployed.
- MOCKDATA + ja-translated Token & Sale Contract Address: Enter the contract addresses provided by Sequence's Builder.
- ja-translated Items For Sale: List all the Token IDs you wish to sell.

## MOCKDATA + ja-translated 4. Implement Custom Code to Interact with the SDK
ja-translated Let's create a custom class to manage the state for our sale. This class will gather all the necessary data,
...
## MOCKDATA + ja-translated 5. Retrieve Your Primary Sale Details
ja-translated Next, use the `ERC1155Sale.cs` reference to obtain sale details and the payment token from the contract.
...
## MOCKDATA + ja-translated 6. Fetching Token Metadata to Display Items to Users
ja-translated We’ll use the Indexer API to retrieve token supplies for the specified token contract address.
...
MOCKDATA + ja-translated For example, you can use the `supply.tokenMetadata.image` variable from the token supplies mentioned above
...
## MOCKDATA + ja-translated 7. Purchase an Item from the Store
ja-translated This will call the mint function from the Sales Contract. By specifying the user’s wallet address as the `to` parameter,
...
MOCKDATA + ja-translated When the user clicks the purchase button in the game UI, we call and await the PurchaseAsync function from the sale state class.
