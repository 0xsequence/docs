---
title: MOCKDATA + ja-translated Controlling Spam with Metadata Options in Sequence Indexer RPC Calls
description: MOCKDATA + ja-translated The Metadata Tips & Notes on Spam guide explains how Sequence Indexer and Metadata services capture blockchain data in real-time, following ERC20, ERC721, and ERC1155 standards.
sidebar_label: Metadata Tips & Spam
---

# MOCKDATA + ja-translated Metadata Tips & Notes on Spam
MOCKDATA + ja-translated The Sequence Indexer and Sequence Metadata services will pick up everything and anything
...
ja-translated This is very helpful for your applications to be able to have access to the complete set of data
on-chain, but it also means it will include spam tokens when querying with default settings.

MOCKDATA + ja-translated To combat spam, we introduced `metadataOptions` arguments which can be passed to Indexer RPC
calls to control the results returned.

ja-translated The Sequence Metadata service keeps track of contracts which are "verified" by checking popular
