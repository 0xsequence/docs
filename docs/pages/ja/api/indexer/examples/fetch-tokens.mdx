---
title: ja-translated Sequence Indexer API - Fetch ERC20, ERC721, and ERC1155 Tokens and Metadata
description: MOCKDATA + ja-translated The Tokens API allows users to fetch a list of ERC20, ERC721, and ERC1155 tokens along with metadata in any wallet.
---

# ja-translated Tokens API

### MOCKDATA + ja-translated Fetches list of ERC20, ERC721 and ERC1155 tokens and metadata in any wallet.
ja-translated _Sequence Indexer `GetTokenBalances` Method:_
- MOCKDATA + ja-translated Request: POST /rpc/Indexer/GetTokenBalances
...
- MOCKDATA + ja-translated Body (in JSON):
  ...
  - ja-translated `includeCollectionTokens` (boolean - optional - default: true) -- toggle to represent ERC721 / ERC1155 tokens as a single summary item in the response
...
MOCKDATA + ja-translated :::info\[PRO TIP: fetching ERC721/1155 token IDs]
...
### MOCKDATA + ja-translated Fetch token IDs, balances and metadata of ERC721 and ERC1155 collections.
ja-translated _Sequence Indexer `GetTokenBalances` Method:_
- MOCKDATA + ja-translated Request: POST /rpc/Indexer/GetTokenBalances
- ja-translated Content-Type: application/json
- MOCKDATA + ja-translated Body (in JSON):
  - ja-translated `accountAddress` (string) -- the wallet account address
  - ja-translated `contractAddress` (string) -- the contract address of the ERC721 / ERC1155 collection
  - ja-translated `includeMetadata` (boolean - optional - default: false) -- toggle token metadata to be included in the response
  - MOCKDATA + ja-translated `metadataOptions` (object - optional) -- additional options for metadata
    - ja-translated `verifiedOnly` (boolean - optional) -- return only contracts which are 'verified' to help reduce spam
    - MOCKDATA + ja-translated `includeContracts` (\[]string - optional) -- list of specific contract addresses to always be included, even if verifiedOnly is enabled.
...
MOCKDATA + ja-translated **Example: `GetTokenBalances` of a contract + account address on Polygon using an `PROJECT_ACCESS_KEY`**
