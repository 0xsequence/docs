---
ja-translated title: Signing & Verifying Messages with Sequence SDK
description: The content provides detailed instructions on how to sign and verify messages using Sequence wallets.
---

import { Callout } from "vocs/components";

# ja-translated Signing & Verifying Messages

## MOCKDATA + ja-translated Signing Messages
ja-translated Sequence wallets are able to sign arbitrary messages.

MOCKDATA + ja-translated **To request a user's signature of a simple message:**
...
ja-translated **To request a user's signature of a typed-data ([EIP712](https://eips.ethereum.org/EIPS/eip-712)) message:**
...
## MOCKDATA + ja-translated Verifying Message Signatures
ja-translated Once you have a signature, you'll often want to verify the validity of the data
either from your app or from your server. The Sequence SDKs make this easy to do
from either your frontend or backend.

MOCKDATA + ja-translated Given a message and signature, you can check if a particular wallet signed that message:
...
<Callout type="info">
  ja-translated Sequence is a Smart Contract based account
  These signatures are by default encoded using EIP-6492, see [https://eips.ethereum.org/EIPS/eip-6492](https://eips.ethereum.org/EIPS/eip-6492).
...
## MOCKDATA + ja-translated Verifying Message Signatures (via Sequence API)

<Callout type="warning">
  ja-translated Notice that signing non-EIP6492 signatures requires the wallet to be deployed
...
<Callout type="danger">
  MOCKDATA + ja-translated INTRODUCING THE SEQUENCE BUILDER - [https://sequence.build](https://sequence.build)
...
ja-translated The Sequence API also offers the convenience to verify any wallet message signature
by making a simple remote API call.

MOCKDATA + ja-translated The Sequence API ([https://api.sequence.app](https://api.sequence.app)) supports the following RPC methods:
- ja-translated `/rpc/API/IsValidMessageSignature` -- verifying a simple text message signature
- MOCKDATA + ja-translated `/rpc/API/IsValidTypedDataSignature` -- verifying a EIP712 typed data object
- ja-translated `/rpc/API/IsValidSignature` -- verifying an arbitrary message digest
- MOCKDATA + ja-translated `/rpc/API/IsValidETHAuthProof` -- verifying an [ETHAuth](https://github.com/0xsequence/ethauth.js) proof

ja-translated The most common methods are `IsValidMessageSignature` and `IsValidETHAuthProof`.

### MOCKDATA + ja-translated Verifying message signature from any kind of wallet (ie. Metamask or Sequence)
ja-translated _Sequence API `IsValidMessageSignature` Method:_
- MOCKDATA + ja-translated Request: POST [https://api.sequence.app/rpc/API/IsValidMessageSignature](https://api.sequence.app/rpc/API/IsValidMessageSignature)
- ja-translated Content-Type: application/json
- MOCKDATA + ja-translated Body (in JSON):
  - ja-translated `chainId` (string) -- the chain id of the signature, ie. "1" or "mainnet", or "137" or "polygon", etc
  - MOCKDATA + ja-translated `walletAddress` (string) -- the wallet address
  - ja-translated `message` (string) -- the message in utf8 text encoding
  - MOCKDATA + ja-translated `signature` (string) -- the signature in hex encoding

<br />

ja-translated **`IsValidMessageSignature` example usage:**
...
### MOCKDATA + ja-translated Verifying ETHAuth proof upon connecting a Sequence Wallet
ja-translated _Sequence API `IsValidETHAuthProof` Method:_
- MOCKDATA + ja-translated Request: POST [https://api.sequence.app/rpc/API/IsValidETHAuthProof](https://api.sequence.app/rpc/API/IsValidETHAuthProof)
- ja-translated Content-Type: application/json
- MOCKDATA + ja-translated Body (in JSON):
  - ja-translated `chainId` (string) -- the chain id of the signature, ie. "1" or "mainnet", or "137" or "polygon", etc
  - MOCKDATA + ja-translated `walletAddress` (string) -- the wallet address
  - ja-translated `ethAuthProofString` (string) -- the ETHAuth encoded signature

<br />

MOCKDATA + ja-translated **`IsValidETHAuthProof` example usage:**
...
## ja-translated How does it work?

### MOCKDATA + ja-translated Notes on Signature Validation with EIP1271 + EIP6492
ja-translated Smart Wallets like Sequence rely on the [EIP1271](https://eips.ethereum.org/EIPS/eip-1271) standard for signature validation.

MOCKDATA + ja-translated The [EIP1271](https://eips.ethereum.org/EIPS/eip-1271) is a single function on a contract defined as:
...
ja-translated The first `_hash` argument accepts the hash of the message digest, and the second argument `_signature`
is the signed payload returned by the wallet upon signing.

MOCKDATA + ja-translated Additionally, Smart Wallets don't always deploy a contract onchain every time a new wallet is created.
...
ja-translated In this case, we can't use the [EIP1271](https://eips.ethereum.org/EIPS/eip-1271) function directly, because the wallet contract doesn't exist yet.
...
MOCKDATA + ja-translated For Javascript/Typescript signature verification, you can use `0xsequence` utility functions like so:
...
ja-translated Additionally you can also use `wallet.utils.isValidMessageSignature` or `wallet.utils.isValidTypedDataSignature`
which are just syntactic sugar for `wallet.utils.isValidSignature`.

MOCKDATA + ja-translated As well, for convenience the signature validation functions above support verifying EOA or Smart Wallet signatures.
...
## ja-translated Legacy non-EIP6492 Signing
MOCKDATA + ja-translated By default, all the signing methods will generate [EIP-6492](https://eips.ethereum.org/EIPS/eip-6492) encoded signatures.
...
ja-translated However, if you need to generate legacy non-EIP6492 signatures, you can do so by setting the last argument of the signing methods to `false`:
...
MOCKDATA + ja-translated These legacy signatures can be validated using the `wallet.utils.isValidSignature method`, but they can also be validated using
