---
title: Building a Sequence Relaying Transaction Server
...
1. [Environment Setup with Express Server](/guides/building-relaying-server#environment-setup-with-express-server): Create a NodeJs based server using the library Express to accept HTTP requests
2. [Project & Access Key Management](/guides/building-relaying-server#project--access-key-management): Claim a public access key to interact with the Sequence stack
3. [Deploy Collectible Contract](/guides/building-relaying-server#deploy-collectible-contract): Deploy a collectible contract in order to be able to submit transactions to the blockchain to mint tokens to a wallet address
4. [Construct Sponsored Relayer with Transactions API](/guides/building-relaying-server#construct-sponsored-relayer-with-the-transactions-api): Craft a function to be used in an Express route to call the Sequence Transactions API from a sponsored contract
