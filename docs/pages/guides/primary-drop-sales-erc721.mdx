...
To create the placeholders, make sure to set your `collectionId` and the `number` of placeholders you want to create inside the `scripts/utils/getBodyAndKeys.js` file, which already contains an example.

<Callout type="warning">
  Make sure to configure your `JWT_SECRET_KEY`, `PROJECT_ACCESS_KEY`, and `PROJECT_ID` in your `.env` file.
</Callout>
...
To reveal the placeholders, make sure to set your `collectionId` inside the `scripts/utils/getBodyAndKeys.js` file, which already contains an example.
