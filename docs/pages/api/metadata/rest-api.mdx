---
title: Sequence Metadata REST API Reference & Usage
...
As well, the Sequence Metadata service is automatically integrated in the [Sequence Indexer](/api/indexer/overview). 
But as we demonstrate below, it's also useful to be able to query the token/contract metadata directly too :)

:::info[PRO TIP: RPC vs REST endpoints]
...
For RPC endpoint see [Token metadata RPC](/api/metadata/token-metadata) and [Contract metadata RPC](/api/metadata/contract-metadata).
:::
