---
title: Sequence Token Metadata Service - ERC721 and ERC1155 Contract Metadata Retrieval
...
Sequence provides a metadata service for fetching token metadata for any ERC721 or ERC1155 contract, on [any EVM chain](/solutions/technical-references/chain-support).

* [Learn how to query token metadata directly for any contract](/api/metadata/token-metadata#fetch-token-metadata-for-any-erc721-or-erc1155-contract)
* [Learn about token metadata standards and formats for ERC721 and ERC1155 tokens](/api/metadata/token-metadata#token-metadata-standards)
...
*Sequence Metadata `refreshContractsTokens` Method:*

* Request: POST /rpc/Metadata/refreshContractTokens
* Content-Type: application/json
* Body (in JSON):
  * `chainID` (string) -- the chain id, as a name or number (ie. "1" or "mainnet", "137" or "polygon", etc.)
  * `contractAddress` (string) -- the contract address
...
```shell [cURL]
curl -v -X POST -H "Content-type: application/json" -H "X-Access-Key: wuELppeX0pttvJABl8bIuxPAAAAAAAAAA" https://metadata.sequence.app/rpc/Metadata/refreshContractsTokens -d '{"chainID":"polygon", "contractAddress":"0x631998e91476DA5B870D741192fc5Cbc55F5a52E", "tokenIDs": ["1","2"]}'
```
```ts [Typescript]
// Works in both a Webapp (browser) or Node.js:
import { SequenceMetadata } from '@0xsequence/metadata'
...
1. Check current token metadata using [token metadata fetch](/api/metadata/token-metadata#fetch-token-metadata-for-any-erc721-or-erc1155-contract)
2. Modify token metadata content either onchain or via token baseURI 
3. Call refresh token ID endpoints using [refresh metadata](/api/metadata/token-metadata#refreshing-token-metadata)
4. Check token metadata changes using [token metadata fetch](/api/metadata/token-metadata#fetch-token-metadata-for-any-erc721-or-erc1155-contract)
