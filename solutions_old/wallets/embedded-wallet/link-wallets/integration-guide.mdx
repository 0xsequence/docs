...
Finally, if you no longer want to keep a wallet linked off-chain to a parent wallet, you can perform a signature by both wallet instances (Embedded Wallet & Web SDK linked wallet), and pass in the linked wallet address, [like in step 6](/solutions_old/wallets/embedded-wallet/link-wallets/integration-guide#6-link-sequence-kit-signed-in-wallet) in order to remove the linked state:
