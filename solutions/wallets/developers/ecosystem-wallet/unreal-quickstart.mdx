...
Next, let's use the permissions object and call one of the authentication functions, such as `Sign In with Google`.
Alternatively, you can leave the permissions empty to create an implicit session.
...
Let's use the transaction object and pass it to the `SendTransaction` function. Optionally, you can [get fee options](/sdk/unreal/ecosystem-wallet/blockchain#get-fee-options) first if you interact
on a mainnet and you don't use gas sponsorship.
