...
Download [v5.0.0 from the SDK's GitHub Releases](https://github.com/0xsequence/sequence-unity/releases/tag/v5-beta)
...
Next, create a `SequenceConfig.asset` file in your Unity project and add the `projectAccessKey` to the configs `Builder API Key` field.
...
Next, let’s use the `SequenceConnect` interface with these permissions to create a wallet session.
