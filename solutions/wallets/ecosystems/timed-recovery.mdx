...
sequenceDiagram
  participant User
  participant WalletApp as Wallet
  participant RecoveryContract as Recovery contract
  participant Recipient as Recipient wallet
  participant EOA as EOA wallet

  User->>WalletApp: Start Recovery (logged out)
  WalletApp->>User: User enters 24 word passphrase
  User-->>WalletApp: Recovery EOA derived and validated
  WalletApp-->>User: Inventory preview
  User->>WalletApp: Provide recipient address
  WalletApp->>RecoveryContract: Queue recovery payload (transfer bundle)
  RecoveryContract-->>WalletApp: Payload queued (timestamped)
  ...
  User->>EOA: At T+30, connect EOA (MetaMask)
  EOA->>RecoveryContract: Submit final proof
  RecoveryContract->>WalletApp: Authorize transfer bundle
...
A: If both main access and the recovery phrase are lost, recovery is not possible. Users are guidede to maintain offline storage plus an encrypted backup.
