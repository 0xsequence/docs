...
And complete your contract details, specifying the `Network`, `Contract Name`, and `Royalties`, whether to publish the Collectible on the [Token Directory](/api-references/indexer/metadata-tips#metadata-tips--notes-on-spam) and finally select `Deploy Contract` to then complete the transaction signing in the Sequence popup.
<Frame>
